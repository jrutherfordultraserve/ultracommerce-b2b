...
public class CheckoutController extends BroadleafCheckoutController {
    ...
    public String checkout(HttpServletRequest request, HttpServletResponse response, Model model,
        ...
        if (CheckoutStageType.PAYMENT_INFO.getType().equals(getActiveCheckoutStage(request))
                && !checkoutFormVariableExpression.shouldShowShippingInfoStage()) {

            paymentInfoForm.setShouldUseShippingAddress(false);
            model.addAttribute("isDigitalCheckout",true);
        } else {
            model.addAttribute("isDigitalCheckout",false);
        }
    ...
    public String getCheckoutStagePartial(HttpServletRequest request, HttpServletResponse response, Model model,
            ...
            RedirectAttributes redirectAttributes) {
        String view = super.getCheckoutStagePartial(request, response, model, activeStage, redirectAttributes);
    ...
    public String processPassthroughCheckout(@RequestParam(value = "guest-checkout", required = false, defaultValue = "false") Boolean isGuestCheckout,
                                             RedirectAttributes redirectAttributes)
            throws PaymentException, PricingException {
        String checkoutPageRedirect = super.processPassthroughCheckout(redirectAttributes, PaymentType.COD);

        if (isGuestCheckout) {
            return checkoutPageRedirect + "?guest-checkout=true";
        }
